...
// KHÔI PHỤC mật khẩu khi quên: đặt env USERS_RESET="vhpro:matkhaumoi,..." rồi redeploy.
// Lúc khởi động sẽ GHI ĐÈ mật khẩu này vào Redis (thắng cả override cũ). Sau khi đăng
// nhập được, NÊN gỡ biến USERS_RESET (nếu để lại, mọi lần đổi mật khẩu trong app sẽ bị
// ghi đè về mật khẩu này ở lần cold-start sau).
async function applyPasswordResets() {
  if (!useRedis) return;
  const resets = {};
  (process.env.USERS_RESET || '').split(',').forEach(p => {
    const i = p.indexOf(':'); if (i > 0) resets[p.slice(0, i).trim()] = p.slice(i + 1).trim();
  });
  const names = Object.keys(resets).filter(u => USERS[u]);   // chỉ account có thật
  if (!names.length) return;
  try {
    const ov = await loadPwOverrides();
    for (const u of names) ov[u] = hashPw(resets[u]);
    await redisSet(PW_KEY, JSON.stringify(ov));
    console.log('Applied USERS_RESET for:', names.join(', '));
  } catch (e) { console.log('USERS_RESET error:', e.message); }
}
applyPasswordResets();
